...
class AiwsMcpSkillTests(unittest.TestCase):
    ...
    def test_resolve_google_drive_marketplace_prefers_current_published_over_stale_cache(self) -> None:
        ...
        with (
            ...
            filtered_workflow = self.runtime.drive_marketplace_workflow(
                marketplace_id="checkout-main-real",
                plugin_id="example-plugin",
                skill_id="meeting-followup",
                host_kind="cowork",
        ...
        self.assertNotIn("selected_action", browse_workflow)
        ...
        self.assertNotIn("selected_action", missing_workflow)
    ...
    def test_clean_machine_has_core_aiws_skills_without_plugins(self) -> None:
        ...
        self.assertIn("aiws-refresh-skill-library", skill_ids)
        ...
        self.assertIn("Save plugin", install)
        self.assertIn(".plugin", install)
        self.assertIn("not a `.skill` artifact", install)
        self.assertIn("Never report `READY FOR SAVE` while the visible action is **Save skill**", install)
        self.assertIn("plugin manifest", install)
        self.assertIn(".claude-plugin/plugin.json", install)
        self.assertIn("contracts/<plugin-id>.contract.json", install)
        self.assertIn("Do not put files under an extra top-level wrapper folder", install)
        self.assertIn("READY FOR SAVE", install)
        self.assertIn("Use `PASS` only after Cowork accepts the plugin", install)
        self.assertIn("Plugin validation failed", install)
        self.assertIn("Do not tell the user to run another prompt in the same Cowork session", install)
        self.assertIn("Do not stop after producing individual", install)
        self.assertIn("Do not call, register, inspect, or repair `aiws.marketplaces.*`", install)
        self.assertIn("Do not mention marketplace in the normal install report", install)
        self.assertIn("Mandatory Self-Improvement", install)
        self.assertIn("Self-Improvement Protocol", install)
        self.assertIn("final phase of the procedure", install)
        self.assertIn("Skill Library Proposal", proposal)
        self.assertIn("propose a meeting-followup update for Test Plugin", proposal)
        self.assertIn("First action should be locating and reading the Drive Skill Library contents", proposal)
        self.assertIn("Do not start by calling AIWS marketplace workflow", proposal)
        self.assertIn("Do not inspect or report AIWS marketplace/materialized state", proposal)
        self.assertIn("Ask for missing information only when the proposal cannot be written safely", proposal)
        self.assertIn("Proposals/Submitted/<skill-id>/<proposal-id>/SKILL.md", proposal)
        self.assertIn("code --diff", proposal)
        self.assertIn("meld", proposal)
        self.assertIn("Mandatory Self-Improvement", proposal)
        self.assertIn("Self-Improvement Protocol", proposal)
        self.assertIn("Skill Library Refresh", refresh)
        self.assertIn("refresh Test Plugin", refresh)
        self.assertIn("Do not interpret these prompts as a request to edit", refresh)
        self.assertIn("canonical already matches", refresh)
        self.assertIn("If installed content already matches Drive canonical content", refresh)
        self.assertIn("no rebuild is required", refresh)
        self.assertIn("present a single **Save plugin** card in the current Cowork session", refresh)
        self.assertIn("run the same artifact preflight as `aiws-install-drive-skill-library`", refresh)
        self.assertIn("Use `READY FOR SAVE`", refresh)
        self.assertIn("Use `NEEDS RETRY` when Cowork produced a **Save skill** card", refresh)
        self.assertIn("Use manual reinstall guidance only if Drive access", refresh)
        self.assertIn("Do not fail a successful refresh only because live skill invocation was not run", refresh)
        ...
        self.assertIn("<plugin-id>--<skill-id>", refresh)
        self.assertIn("missing `plugins/` folder", refresh)
        self.assertIn("whole Cowork plugin artifact", refresh)
        self.assertIn("Mandatory Self-Improvement", refresh)
        self.assertIn("Self-Improvement Protocol", refresh)
        self.assertIn("Skill Library Update", update)
        self.assertIn("Compatibility alias", update)
        self.assertIn("maintainer-applied update", update)
        self.assertIn("update Test Plugin skill library", update)
        self.assertIn("verify/refresh the library by default", update)
        self.assertIn("canonical is already in sync", update)
        self.assertIn("no rebuild is required", update)
        self.assertIn("following `aiws-refresh-skill-library` semantics", update)
        self.assertIn("rebuild/preflight/present a **Save plugin** card", update)
        self.assertIn("Use `READY FOR SAVE`", update)
        self.assertIn("Do not fail a successful update/refresh only because live skill invocation was not run", update)
        self.assertIn("apply mode", update)
        self.assertIn("Proposals/Submitted/<skill-id>/<proposal-id>/SKILL.md", update)
        ...
        self.assertIn("Mandatory Self-Improvement", update)
        self.assertIn("Self-Improvement Protocol", update)
        self.assertIn("Skill Library Validation", validation)
        self.assertIn("Validate the Test Plugin Drive library and include installed plugin status", validation)
        self.assertIn("ambiguous in Cowork", validation)
        self.assertIn("validate-skill-library", validation)
        self.assertIn("the Drive folder is the source of truth", validation)
        ...
        self.assertIn("Mandatory Self-Improvement", validation)
        self.assertIn("Self-Improvement Protocol", validation)
        ...
        self.assertIn("Mandatory Self-Improvement", check)
        self.assertIn("Self-Improvement Protocol", check)
