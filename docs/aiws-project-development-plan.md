...
For this urgent MVP, staging means calling the contract-owned `stage_proposal(draft_id, target_scope, target_repo, summary, rationale)` operation to write a local proposal record with provenance, the concrete backend review repository, and review notes under `~/.aiws/state/skill-proposals/`. `target_scope` is the Cowork/user-facing destination label and policy scope; `target_repo` is the concrete repository used later by submit-for-review. Staging must not be silently treated as PR submission. Submission is a separate explicit Cowork UI action that may create or update a GitHub pull request behind the scenes. Normal users should see statuses such as `Draft`, `Modified locally`, `Ready to submit`, `Submitted for review`, `Changes requested`, `Accepted`, `Released`, and `Marketplace synced`, not raw git mechanics. `Accepted` means the skill proposal merged; it does not mean the updated plugin is visible in Cowork.

GitHub is now an acceptable collaboration backend for the customer experiment. The product boundary is still clear: Cowork owns the normal user workflow for staging and submitting; GitHub owns maintainer review, comments, approvals, merges, and release PRs. Direct push is not part of the normal user flow. Pull requests are created by an explicit submit action and should target the appropriate unit, company, public, or personal skills repository based on the selected target scope. After a skill proposal is accepted, maintainers release plugin versions through the reusable AIWS repo scaffold: a GitHub App-backed Actions workflow opens a release PR that updates plugin manifest, contract, and marketplace plugin metadata consistently before Cowork marketplace sync.
...
- Participating repos can use the reusable AIWS scaffold to generate maintainer release PRs with consistent plugin version, contract, package, and marketplace metadata updates.
