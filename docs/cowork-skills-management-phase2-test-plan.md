...
Start from Cowork marketplace install. For this Phase 2A retest, `core-aiws` must be refreshed to version `0.3.8` or newer, because earlier installs either did not bundle the AIWS MCP bridge or did not include the Scenario D draft-record safety fix.
