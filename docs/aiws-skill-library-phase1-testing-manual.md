...
Resolved upstream as of core-aiws v0.5.x source repo (pending next plugin release):

- ~~`aiws-install-drive-skill-library` SKILL.md hard-codes a preflight expectation that `skills/meeting-followup/SKILL.md` exists in the demo library.~~ **Fixed** in commit `8e037cc` (2026-05-28) — SOP audit pass removed hard-coded skill ids across all Drive-library skills. Preflight is now data-driven from the actual Drive listing.
- ~~`aiws-propose-skill-update` SKILL.md instructs the maintainer to update `contracts/test-plugin.contract.json` on accept.~~ **Fixed** in commit `8e037cc` (2026-05-28) — the contract-edit instruction was removed from the maintainer-accept guidance. The file is no longer mentioned in the propose SKILL.md.
- ~~`aiws-refresh-skill-library` SKILL.md does not include the same anti-marketplace guard that `aiws-install-drive-skill-library` does.~~ **Fixed** (Phase 2 priority-1) — refresh now mirrors the install skill's anti-marketplace-fallback wording in its Boundaries section: "A flat `skills/<skill-id>/SKILL.md` Drive folder is valid even if AIWS marketplace indexing would return no results. Do not use missing marketplace search results, an empty `<plugin-id>` marketplace, or absent materialized skills as evidence that the library cannot be refreshed — read Drive directly and proceed."

Additional hardening from Phase 2 priority-1:

- `aiws-install-drive-skill-library` SKILL.md now includes a Source-Shape Validation section that rejects Drive roots containing `.claude-plugin/` or `contracts/` at top level as "not a Drive Skill Library", instead of silently packaging only the `skills/` subset. Fail-fast prevents contract drift between source and packaged artifact.

Take effect on next core-aiws plugin release. Smoke-verification against `Test Plugin` Drive library is release-gated.
