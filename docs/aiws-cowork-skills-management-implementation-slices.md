...
Implementation and runtime update in `core-aiws` 0.3.17: a GitHub REST API submitter can create/update proposal branches and PRs without host `gh` when the host provides a GitHub token through environment/configuration. Runtime selection prefers the API submitter when a token is present, keeps `gh` as technical-pilot fallback, and preserves `submit_handoff_required` when no submit path is available. Cowork Scenario 12B passed and created PR #6 in `sashakang/aiws-skill-tests` through the API path. See [Cowork GitHub API Submitter PASS](./cowork-github-api-submitter-pass-2026-05-15.md).
