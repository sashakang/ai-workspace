...
- `contracts/` — the same indicator. `contracts/<plugin-id>.contract.json` is an artifact output generated by the install/refresh packager, not a Drive Skill Library input. Its presence in the source means the user pointed at a packaged plugin tree by mistake.

Do not silently package only the `skills/` subset of such a tree — that hides the real misconfiguration and produces a plugin whose contract will drift from the source. Stop and report.
